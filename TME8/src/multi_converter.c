...
int main(int argc, char **argv){
	...
	sigfillset(&sig2);
	sigdelset(&sig2, SIGUSR1);
	
	sigact2.sa_mask = sig2;
	sigact2.sa_flags = 0;
	sigact2.sa_handler = g;
	sigaction (SIGUSR1, &sigact2, &old2);
	...
	while(1){
		...
		for(i = 0; i < NB_CONVERTERS; i++){
			...
			if(fork() == 0){
				...
				if(write(tube[1], &rep[i], sizeof(conversion_message)) == -1){
					perror("write");
					return EXIT_FAILURE;
				}
				...
				if(read(tube[0], &result[i], sizeof(conversion_message)) == -1){
					perror("read");
					return EXIT_FAILURE;
				}
			}
		...
		if(write(fd_write, result, sizeof(results_array)) == -1){
			perror("write");
			return EXIT_FAILURE;
		}
